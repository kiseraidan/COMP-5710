...
def trackTaint(val2track, df_list_param): 
    ...
    taint_track = {}
    for _, row in var_.iterrows():
        if isinstance(row['RHS'], int):
            taint_track[row['RHS']] = row['LHS']
        elif isinstance(row['RHS'], str) and "," not in row['RHS']:
            taint_track[row['RHS']] = row['LHS']
        elif isinstance(row['RHS'], str):
            for sub_var in row['RHS'][1:].split(","):
                taint_track[sub_var] = row['LHS']
    func_track = {}
    for _, row in func_def.iterrows():
        func_track[row['TYPE'][-1]] = row['ARG_NAME']
    trace = [str(val2track)]
    data = val2track
    while taint_track.get(data):
        trace.append(str(taint_track[data]))
        data = taint_track[data]
    trace.append("res")
    trace = trace[-3:]
    # Add v1
    if 'v1' in taint_track:
        trace.insert(2, 'v1')
    print("->".join(trace)) 
